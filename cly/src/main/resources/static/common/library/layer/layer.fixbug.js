...
Class.pt.vessel = function(conType, callback){
    ...
    var marginSetStr = (((config.type===1)&&config.marginSet!=undefined)?' style="padding:'+config.marginSet+';"' : '') ;
...
Class.pt.move = function(){
    ...
    $(document).mousemove(function(M){
        ...
        config.moveEnd && config.moveEnd(that.index, that.layero);
    });
    return that;
};
...
Class.pt.callback = function(){
    ...
    if(config.type == 2){
    	...
    	var fid = layero.find('iframe')[0].contentWindow.name ;
    	layer.frameData[fid] = {index:that.index} ;
    	if(initData!=null){
    		if(typeof initData == "function"){
    			var initData1 = initData() ;
    			layer.frameData[fid].initData = initData1 ;
    		}else{
    			layer.frameData[fid].initData = initData ;
    		}
    	}
    }
    if(config.success){
...
layer.close = function(index){
    ...
    if(type === ready.type[2]){
    	var fid = layero.find('iframe')[0].contentWindow.name ;
    	if(layer.frameData[fid]!=null){
    		delete layer.frameData[fid] ;
    	}
    }
