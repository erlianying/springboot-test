...
(function( $ ) {
   ...
   $.fn.create=function(/*selector [,eventData] ,handler [,noExisting]*/) {
      ...
      if (useBehaviors) {
    	  if(document.getElementById('jcade.create.htc').styleSheet==undefined){
   		   	 if(myStyleSheet==null){
	        	 myStyleSheet = document.styleSheets[0];       	 
	         }
   		   	 myStyleSheet.addRule(selector,'behavior:url('+$.fn.create.htcPath+'jcade.create.htc)',0);
    	  }else{
...
(function($) {
   ...
   $.fn.destroy=function(/*[ eventData,] handler, namespace*/) {
      ...
      if (useBehaviors) {
         ...
         $(document).on('DOMNodeRemoved.jcade'+(namespace ? '.'+namespace : ''), arguments[3], function(event) {
