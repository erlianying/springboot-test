...
(function($){
    ...
    $(document).ready(function() {
        ...
        $(document).on("ifChecked","input[name='chartsTimeDay']",function () {
            ...
            var startTimeLong = endTimeLong - type * 24 * 3600 * 1000;
        ...
        $(document).on("ifChecked","input[name='chartsTimeHour']",function () {
            ...
            $.laydate.setRange(startTimeStr, endTimeStr, "#dateRangeHour");
        });
        ...
        $(document).on("ifChecked","input[name='dayTimeSelect']",function () {
            ...
            if(type != ""){
                ...
                var startTimeStr = $.date.dateToStr(new Date(startTimeLong), "yyyy-MM-dd " + type);
         ...
         * 时间修改
         */
        $(document).on("change","#fixed_startA",function (){
            var day = $($.icheck.getChecked("chartsTimeDay")[0]).val();
            var startTimeLong = $.laydate.getTime("#dayTimeDiv", "start");
            var endTimeLong = startTimeLong + day * 24 * 3600 * 1000;
            var endTimeStr = $.date.dateToStr(new Date(endTimeLong), "yyyy-MM-dd HH");
            $("#fixed_endA").val(endTimeStr);
        });
        $(document).on("change","#fixed_endA",function (){
            var day = $($.icheck.getChecked("chartsTimeDay")[0]).val();
            var endTimeLong = $.laydate.getTime("#dayTimeDiv", "end");
            var startTimeLong = endTimeLong - day * 24 * 3600 * 1000;
            var startTimeStr = $.date.dateToStr(new Date(startTimeLong), "yyyy-MM-dd HH");
            $("#fixed_startA").val(startTimeStr);
        });
        $(document).on("change","#fixed_startB",function (){
            var hour = $($.icheck.getChecked("chartsTimeHour")[0]).val();
            var startTimeLong = $.laydate.getTime("#hourTimeDiv", "start");
            var endTimeLong = startTimeLong + hour * 3600 * 1000;
            var endTimeStr = $.date.dateToStr(new Date(endTimeLong), "yyyy-MM-dd HH");
            $("#fixed_endB").val(endTimeStr);
        });
        $(document).on("change","#fixed_endB",function (){
            var hour = $($.icheck.getChecked("chartsTimeHour")[0]).val();
            var endTimeLong = $.laydate.getTime("#hourTimeDiv", "end");
            var startTimeLong = endTimeLong - hour * 3600 * 1000;
            var startTimeStr = $.date.dateToStr(new Date(startTimeLong), "yyyy-MM-dd HH");
            $("#fixed_startB").val(startTimeStr);
        });
    ...
    function setDefaultTime() {
        ...
        var startDayDateTime = nowDate.getTime() - 10 * 24 * 3600 * 1000;
