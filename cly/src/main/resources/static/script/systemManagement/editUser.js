...
(function($){
    ...
    function init(){
        ...
        $.ajax({
            ...
            dataType:"json",
            success:function(map){
                $("#accountName").val(map.pojo.accountName);
                $("#password").val(map.pojo.password);
                $("#person").text(map.pojo.person);
                personId = map.pojo.personId;
                window.setTimeout(function(){
                    $("#status").select2("val",map.pojo.statusCode);
                },1000);
                $("#organizationName").text(map.pojo.organizationName);
                $("#start").val(map.pojo.startDateStr);
                $("#end").val(map.pojo.endDateStr),
                $("#intro").val(map.pojo.intro);
                $.fn.zTree.init($("#tree-right"), roleSetting, map.roleLis);
            }
        })
    }
    ...
    function edit(){
        ...
        $.ajax({
            url:context + '/account/updateAccount',
            type:"post",
            data:obj,
