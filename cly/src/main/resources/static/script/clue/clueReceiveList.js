(function($){
    ...
    $(document).ready(function() {
        ...
        if( ($.util.isBlank($("#isQuery")) || $("#isQuery").val() == "" || $("#isQuery").val() == "false") && !$.util.isBlank(startTime)){     //图跳转过来的查询 20180131LGQ
            initData();
            initClueTable();
    ...
    function events(){
            ...
            $.ajax({
                ...
                success:function(successData){
                    if(successData){
                        //治安 二支队 主责单位可以办结
                        $.ajax({
                            url:context +'/clueFlow/canEndProcessing',
                            type:'post',
                            data:{clueId : clueId},
                            dataType:'json',
                            success:function(successData){
                                if(successData){
                                    ...
                                    window.top.$.layerAlert.alert({msg:"只有治安总队、网安总队二支队和主责单位可以办结！",icon:"1",end : function(){
                                    }});
                                }
                            }
                        });
        ...
        $(document).on("click",".query",function(){
            startTime = null;       //查询后不再有跳转前的条件 20180131LGQ
            $("#isQuery").val(true);
            $(".clearSelect").click();
            initClueTable();
        });
        $(document).on("click",".showUploadDialog",function(){
            var tranData = {
                downloadUrl : '/petitionRegistrationExportExcel/downloadPetitionModel?fileName=relevantPersonInfo.xlsx',
                uploadUrl : '/clue/cluePersonInfoBatchImport',
                win$ : $
            };
            showUploadDialog(tranData);
        });
        $(document).on("click",".showCheckUploadDialog",function(){
            var tranData = {
                downloadUrl : '/petitionRegistrationExportExcel/downloadPetitionModel?fileName=checkImportModel.xlsx',
                uploadUrl : '/clue/clueCheckBatchImport',
                win$ : $
            };
            showUploadDialog(tranData);
        });
        ...
        $(document).on("click",".changePage",function(){
            ...
            var maxPage = parseInt( (totalNum+9)/10 );
            if(maxPage == 0) {
                maxPage = 1;
            }
            ...
            $("#clueTable").DataTable().page(pageNumber - 1).draw(false);
        });
    }

    function showUploadDialog(tranData){
        window.top.$.layerAlert.dialog({
            content : context +  '/show/page/web/clue/batchImportLayerForPersonInfo',
            pageLoading : true,
            title:"导入",
            width : "500px",
            height : "350px",
    ...
    function initData(){
        ...
        $.ajax({
            ...
            success:function(successData){
                ...
                if($("#queryStatusInput").val() == "clueQuery"){
    ...
    function initClueTable(){
        ...
        var functionName = "";
        if( ($.util.isBlank($("#isQuery")) || $("#isQuery").val() == "" || $("#isQuery").val() == "false") && !$.util.isBlank(startTime)){     //图跳转过来的查询 20180131LGQ
            functionName = "/clueFlow/findClueListForLittleWindowForReceive";
        }else if(queryStatus == "fullQuery"){
            functionName = "/clue/findCluePageByFullForReceive";
        }else if(queryStatus == "clueQuery"){
            functionName = "/clue/findCluePageByClueForReceive";
        }
        ...
        tb.columnDefs = [
            ...
            },
            {
                ...
                "render": function ( data, type, full, meta ) {
                    var str = "";
                    if(full.point == $.common.dict.SF_S){
                        str += '<span class="fa fa-check-square-o" clueId="' + data + '" style="color:orange"></span>';
                    }else{
                        str += '<span clueId="' + data + '" style="margin-right: 14px;"></span>';
                    }
                    if(full.checkFlag == $.common.dict.SF_S){
                        str += '<span class="fa fa-bookmark fc-blue" clueId="' + data + '"></span>';
                    }else{
                        str += '<span clueId="' + data + '" style="margin-right: 14px;"></span>';
                    }
                    var level = "";
                    ...
                    str += '<span class="fa fa-star ' + level + '" clueId="' + data + '"></span>' + (meta.row + 1);
                    return str;
                }
            },
            {
            ...
            },
            {
                ...
                "data" : "feedbackFlag",
                "render" : function(data, type, full, meta) {
                    if(full.status != $.common.dict.XSZT_YBJ){
                        var str = '<p class="text-center">';
                        // if(data == "true"){ TODO 两会期间停用反馈，之后改回来
                        //     str += '<a href="' + context + '/show/page/web/clue/feedbackState?clueId=' + full.id + '" class="btn btn-default btn-xs">反馈</a>';
                        // }else{
                            str += '<a href="' + context + '/show/page/web/clue/checkClue?clueId=' + full.id + '" class="btn btn-default btn-xs">核查</a>';
                        //}
                        str += '</p>';
                        return str;
                    }else{
    ...
    function exportExcel(btnType){
        ...
        if(!$.util.isBlank(startTime)){     //图跳转过来的结果导出 20180131LGQ
            url = context +  "/clue/exportClueListForLittleWindowExcelForReceive";
            var obj = {};
            obj.targetSiteSite = targetSite;
            obj.startTimeOneLong = startTime;
            obj.startTimeTwoLong = endTime;
            ...
            url = context + "/clue/exportCluePageByFullForReceive";
            var obj = {};
            ...
            url =context +  "/clue/exportCluePageByClueForReceive";
            var obj={};
            obj.content = $("#clueContent").val();
            obj.doneTimeOneLong = $.laydate.getTime("#doneTime", "start");
            obj.doneTimeTwoLong = $.date.endRangeByTime($.laydate.getTime("#doneTime", "end"),"yyyy-MM-dd HH:mm");
